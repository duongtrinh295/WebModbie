...
function doChooseVariant(pv){
    ...
    function doChooseColor(color_item){
        ...
        if($(color_item).hasClass('active')){
            ...
            // $(color_item).closest('.color-list').find('.color-name-selected').css('display','none');
    ...
    function autoSelectCheapestVariant(){
        try {
            let variants = pv.product_variants;
...
function showInfoVariantSync(product_variants, extra_prices, product_id, instock_status){
    class ProductVariant{
        ...
        button_buy_now = `<a href="javascript::" data-product_type="tablet_variants" class="buy buy-now v2-buy">MUA NGAY</a>`;
        button_add_to_cart = `<a href="javascript::" data-product_type="tablet_variants" class="buy add-to-cart v2-buy"><div class="icon"></div><span>THÊM VÀO GIỎ HÀNG</span></a>`;
        button_out_stock = `<a href="javascript::"  class="buy-now disabled">Hết hàng</a>`;
        button_register = `<a href="#" class="buy register" data-product_type="tablet_variants">ĐẶT HÀNG TRƯỚC</a>`;
        button_chat = `<a href="${$('.mobilecity-messenger').attr('href') ?? 'javascript:;'}" rel ="nofollow" target="_blank" class="chat">CHAT TƯ VẤN</a>`;
        button_pre_order = `<a href="javascript:;" data-product_id="${ product_id }" data-variant_id="0" data-service_product_id="0" class="pre-order" data-product_type="tablets">ĐẶT HÀNG TRƯỚC</a>`;
        ...
        constructor(product_variants, extra_prices, product_id = 0, instock_status){
            ...
            this.product_variant_ids = this.product_variants.map(function(variant){
        ...
        getInstock(){
            ...
            loadAjax('/ajax/check-instock-all-location', {
                'product_id': pv.product_variant_ids,
                ...
                success: function(result) {
                    if(!result.success){
                        ...
                        pv.error = 1;
                        updateIsStockProduct(product_id, 0);
                    }else{
                        ...
                        updateIsStockProduct(product_id, is_stock);
                    }
        ...
        mixInstockDataAndVariant(){
            this.instocks.forEach(element => {
                let variant_tmp = this.product_variants.find(function(v) {
        ...
        getCurrentVariant(){
            ...
            this.getCurrentAttribute();
            let current_variant = this.product_variants.find(function(variant){
        ...
        getSelectedVariants(){
            this.getCurrentAttribute();
            return this.product_variants.filter(function(variant){
    ...
    pv = new ProductVariant(product_variants, extra_prices, product_id, instock_status);
...
function showInfoVariantNotSync(product_variants, extra_prices, product_id, instock_status){
    class ProductVariant{
        product_variants = {};
        extra_prices = {};
        div_product = $('.product-content-box');
        ...
        constructor(product_variants, extra_prices, product_id, instock_status){
            var is_stock = 0;
            if (instock_status == 1) {
                is_stock = 1;
            }
...
function responsiveRelatedProductDesktop(){
    ...
    // console.log(height_related_product, height_product_info);
...
// function showRelatedProduct(product_id){
//     // load DT
//     loadAjaxGet(`/ajax/phone-get-list-related-phone/${product_id}`,{
//         success: function(result) {
//             if(!result.success){
//                 showRelatedFit(product_id);
//                 return;
//             }
...
// function showRelatedFit(product_id){
//     loadAjaxGet(`/ajax/phone-get-list-related-fitting/${product_id}`,{
//         success: function(result) {
//             if(!result.success){
...
function updateIsStockProduct(product_id, is_stock){
    $.ajax({
